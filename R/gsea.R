...
gsea <- function(rl=NULL, gsl=NULL, k=99, min_size=5, max_size=500, min_tags=3, abs_val=NULL, decreasing=NULL, BPPARAMGsl=NULL, BPPARAMK=NULL, description=NULL, out_file_prefix="gsea_res", min.k=50){
  ...
  res <- bplapply(gsl, function(x) do.call(rbind, bplapply(x_perm, function(y) calc_gs_perm(rll, y, x), BPPARAM = BPPARAMK)), BPPARAM = BPPARAMGsl)
  ...
  for(i in 1:length(rll)){
    ...
    #out table
    out[[i]] <- data.frame(id=rownames(res[[i]]), size=gsl_size[match(rownames(res[[i]]), names(gsl_size))], es=res[[i]][, 1], nes=nes[, 1], nperm=nperm, p_val=p_val, adj_p_val=p.adjust(p_val, method='fdr'), q_val=qvalues, FDRq=fdrq, description=description[match(rownames(res[[i]]), names(description))], stringsAsFactors=FALSE)
